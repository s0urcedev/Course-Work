...
<svelte:head>
    <title>Оберіть тест</title>
</svelte:head>

<main>
    {#if usersTests !== undefined && usersTests !== null}
        {#if usersTests.length > 0}
            <span style="font-size: 28px">Оберіть тест:</span>
            {#each usersTests as test}
                <a style="color: #009075" href="/uk/testing/{link}?id={test['_id']}">
            ...
            {/each}
        {:else}
            <span style="font-size: 28px">Ви не маєте тестів</span>
        {/if}
    {/if}
</main>